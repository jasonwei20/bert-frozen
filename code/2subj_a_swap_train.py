...
data_folder = config.data_folders['subj']
...
if __name__ == "__main__":
    ...
    for alpha in [0.3]: #0.5, 0.4, 0.3, 0.2, 0.1, 0.05]: 

        print(f"\nalpha={alpha}")

        delete_folder = utils_config.make_exp_folder(data_folder, f"delete{alpha}")
        ...
        eda_folder = utils_config.make_exp_folder(data_folder, f"eda{alpha}")
    ...
    #     # utils_bert.get_embedding_dict(swap_train_txt_path, swap_train_embedding_path)
        ...
        utils_svm.evaluate_svm_big_ablation(train_txt_path,
                                            ...
                                            train_embedding_path,
                                            test_embedding_path,
                                            insert_train_txt_path = delete_train_txt_path,
                                            # insert_test_txt_path,
                                            insert_train_embedding_path = delete_train_embedding_path,
                                            # insert_test_embedding_path,
                                            swap_train_txt_path = eda_train_txt_path,
                                            # swap_test_txt_path,
                                            swap_train_embedding_path = eda_train_embedding_path,
                                            # swap_test_embedding_path,
                                            )